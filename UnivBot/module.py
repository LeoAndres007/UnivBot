# coding=utf8
"""This module is meant to be imported from UnivBot modules.

It defines the following decorators for defining UnivBot callables:
UnivBot.module.rule
UnivBot.module.thread
UnivBot.module.name (deprecated)
UnivBot.module.commands
UnivBot.module.nickname_commands
UnivBot.module.priority
UnivBot.module.event
UnivBot.module.rate
UnivBot.module.example
"""
...
import UnivBot.test_tools
import functools

NOLIMIT = 1
"""Return value for ``callable``\s, which supresses rate limiting for the call.

*Avalability: 4.0+; available as ``UnivBot.NOLIMIT`` in 3.2*
...
def rule(value):
    ...
    If the UnivBot instance is in a channel, or sent a PRIVMSG, where a string
...
def event(value):
    ...
    (More details can be found in RFC 1459.) When the UnivBot bot is sent one of
...
def rate(value):
    ...
    on the admin list in UnivBot’s configuration are exempted from rate limits.
