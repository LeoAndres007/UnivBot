...
import UnivBot.irc as irc
from UnivBot.db import UnivBotDB
from UnivBot.tools import (stderr, Nick, PriorityQueue, released,
                   get_command_regexp, iteritems, itervalues)
import UnivBot.module as module
...
class UnivBot(irc.Bot):
    NOLIMIT = module.NOLIMIT

    def __init__(self, config):
        irc.Bot.__init__(self, config.core)
        self.config = config
        """The ``Config`` for the current UnivBot instance."""
        self.doc = {}
        """
        ...
        self.memory = tools.UnivBotMemory()
        """
        A thread-safe dict for storage of runtime data to be shared between
        modules. See `UnivBotMemory <#tools.UnivBot.UnivBotMemory>`_
        """

        self.scheduler = UnivBot.JobScheduler(self)
    ...
    class JobScheduler(threading.Thread):
        ...
        def _call(self, func):
            ...
            # UnivBot.bot.call is way too specialized to be used instead.
            try:
                func(self.bot)
            except Exception:
                self.bot.error()
    ...
    def setup(self):
        stderr("\nWelcome to UnivBot. Loading modules...\n\n")
        self.callables = set()
    ...
    @staticmethod
    def is_callable(obj):
        """Return true if object is a UnivBot callable.
        ...
        as a UnivBot callable.

        """
        if not callable(obj):
            # Check is to help distinguish between UnivBot callables and objects
    ...
    @staticmethod
    def is_shutdown(obj):
        """Return true if object is a UnivBot shutdown method.
    ...
    def register(self, variables):
        """Register all UnivBot callables.

        With the ``__dict__`` attribute from a UnivBot module, update or add the
        trigger commands and rules, to allow the function to be triggered, and
        shutdown methods, to allow the modules to be notified when UnivBot is
        quitting.
    ...
    def unregister(self, variables):
        """Unregister all UnivBot callables in variables, and their bindings.
        ...
        Args:
        variables -- A list of callable objects from a UnivBot module.
        ...
        UnivBot = self.UnivBotWrapper(self, irc.Origin(self, hostmask, [], {}))
        for obj in itervalues(variables):
        ...
        for func in self.callables:
            ...
            if hasattr(func, 'interval'):
                for interval in func.interval:
                    job = UnivBot.Job(interval, func)
                    self.scheduler.add_job(job)

    class UnivBotWrapper(object):
        def __init__(self, UnivBot, origin):
            object.__setattr__(self, 'bot', UnivBot)
    ...
    class Trigger(unicode):
        def __new__(cls, text, origin, bytes, match, event, args, self):
            ...
            """
            True if the nick which triggered the command is in UnivBot's admin
    ...
    def call(self, func, origin, UnivBot, trigger):
        nick = trigger.nick
        ...
        try:
            exit_code = func(UnivBot, trigger)
        except Exception:
            exit_code = None
    ...
    def dispatch(self, origin, text, args):
        ...
        wrapper = self.UnivBotWrapper(self, origin)
    ...
    def debug(self, tag, text, level):
        """Sends an error to UnivBot's configured ``debug_target``.
    ...
    def cap_req(self, module_name, capability, failure_callback):
        """Tell UnivBot to request a capability when it starts.
        ...
        The arguments will be a `UnivBot` object, and the capability which was
