...
import codecs
import traceback
from UnivBot.tools import stderr, Nick
try:
    import select
    import ssl
    has_ssl = True
...
class Bot(asynchat.async_chat):
    def __init__(self, config):
        ...
        """UnivBot's current ``Nick``. Changing this while UnivBot is running is
        untested."""
        self.user = config.user
        """UnivBot's user/ident."""
        self.name = config.name
        """UnivBot's "real name", as used for whois."""

        self.channels = []
        """The list of channels UnivBot is currently in."""
    ...
    def log_raw(self, line, prefix):
        ...
        if not os.path.isdir(self.config.core.logdir):
            try:
                ...
                stderr('Please fix this and then run UnivBot again.')
                os._exit(1)
