...
*Note:* This supersedes the ``SettingsDB`` object of 3.0. Within UnivBot
...
from UnivBot.tools import deprecated
...
class UnivBotDB(object):

    """UnivBotDB object configured with the options in the given Config object.

    Return a UnivBotDB object configured with the options in the given Config
    ...
    def __nonzero__(self):
        """Allow for testing if a db is set up through `if UnivBot.db`."""
    ...
    def check_table(self, name, columns, key):
        ...
        Return ``True`` if the UnivBotDB contains a table with the same ``name``
    ...
    def add_table(self, name, columns, key):
        """Add a table to UnivBotDB according to the given parameters.
        ...
        UnivBotDB object. If a table with the same name and key already exists,
        ...
        (with the exception of other tables) of the ``UnivBotDB`` object, nor
        ...
        custom query, and then creating the UnivBotDB object.
...
class Table(object):
    ...
    """Return an object which represent a table in the given UnivBotDB.

    Return an object which represents a table in the given UnivBotDB, with the
    ...
    def get(self, row, columns, key=None):
        """Equivalent to SELECT FROM WHERE for UnivBotDB.
    ...
    def update(self, row, values, key=None):
        """Equivalent to UPDATE SET WHERE for UnivBotDB.
    ...
    def delete(self, row, key=None):
        """Equivalent to DELETE FROM WHERE for UnivBotDB.
