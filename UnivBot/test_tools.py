...
test_tools.py - UnivBot misc tools
Copyright 2013, Ari Koivula, <ari@koivu.la>
Licensed under the Eiffel Forum License 2.

https://UnivBot.dftba.net
"""
...
import UnivBot.config
import UnivBot.bot
import UnivBot.irc
import UnivBot.tools


class MockUnivBot(object):
    def __init__(self, nick, admin=False, owner=False):
        ...
        self.memory = UnivBot.tools.UnivBotMemory()

        self.ops = {}
        self.halfplus = {}
        self.voices = {}

        self.config = UnivBot.config.Config('', load=False)
        self._init_config()
    ...
    def _init_config(self):
        ...
        home_dir = os.path.join(os.path.expanduser('~'), '.UnivBot')
...
class MockUnivBotWrapper(object):
...
def get_example_test(tested_func, msg, results, privmsg, admin,
    ...
    Args:
        tested_func - A UnivBot callable that accepts UnivBotWrapper and Trigger.
    ...
    """
    def test():
        bot = MockUnivBot("NickName", admin=admin, owner=owner)

        match = None
        if hasattr(tested_func, "commands"):
            for command in tested_func.commands:
                regexp = UnivBot.tools.get_command_regexp(".", command)
        ...
        origin = UnivBot.irc.Origin(bot, hostmask, origin_args, tags)
        trigger = UnivBot.bot.UnivBot.Trigger(
        ...
        for _i in range(repeat):
            wrapper = MockUnivBotWrapper(bot, origin)
