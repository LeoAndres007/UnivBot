...
import sys
from UnivBot.tools import stderr

if sys.version_info < (2, 7):
    stderr('Error: Requires Python 2.7 or later. Try python2.7 UnivBot')
    sys.exit(1)
...
from UnivBot.__init__ import run
from UnivBot.config import Config, create_config, ConfigurationError, wizard
import UnivBot.tools as tools
import UnivBot.web

homedir = os.path.join(os.path.expanduser('~'), '.UnivBot')

def enumerate_configs(extension='.cfg'):
    configfiles = []
    if os.path.isdir(homedir):
        UnivBot_dotdirfiles = os.listdir(homedir)  # Preferred
        for item in UnivBot_dotdirfiles:
...
def main(argv=None):
    ...
    try:
        ...
        parser.add_argument("-d", '--fork', action="store_true",
                          dest="deamonize", help="Deamonize UnivBot")
        parser.add_argument("-q", '--quit', action="store_true", dest="quit",
                          help="Gracefully quit UnivBot")
        ...
        try:
            if os.getuid() == 0 or os.geteuid() == 0:
                stderr('Error: Do not run UnivBot with root privileges.')
                sys.exit(1)
        ...
        if opts.config is None:
            pid_file_path = os.path.join(pid_dir, 'UnivBot.pid')
        else:
            basename = os.path.basename(opts.config)
            if basename.endswith('.cfg'):
                basename = basename[:-4]
            pid_file_path = os.path.join(pid_dir, 'UnivBot-%s.pid' % basename)
        if os.path.isfile(pid_file_path):
            ...
            if tools.check_pid(old_pid):
                if not opts.quit and not opts.kill:
                    stderr('There\'s already a UnivBot instance running with this config file')
                    stderr('Try using the --quit or the --kill options')
                    sys.exit(1)
                elif opts.kill:
                    stderr('Killing the UnivBot')
                    os.kill(old_pid, signal.SIGKILL)
                    sys.exit(0)
                elif opts.quit:
                    stderr('Signaling UnivBot to stop gracefully')
        ...
        # Step Five: Initialise And Run UnivBot
        run(config_module)
