.. UnivBot IRC Bot documentation master file, created by
...
different versions of it floating around. This documentation focuses on UnivBot,
...
fork thereof considered version *2.x*. UnivBot versions start at *3.0* and
follow `semantic versioning <http://semver.org>`_.

.. contents:: :depth: 2

Getting started: Your functions, ``UnivBot``, and ``trigger``
============================================================

At its most basic, writing a UnivBot module involves creating a Python file with
some number of functions in it. Each of these functions will be passed a 
``UnivBot`` object (``Phenny`` in *1.x* and ``Jenni`` in *2.x*) and a ``Trigger``
object (``CommandInput`` in *1.x* and *2.x*). By convention, these are named
``phenny`` and ``input`` in *1.x*, ``jenni`` and ``input`` in *2.x*, 
``UnivBot`` and ``trigger`` in *3.x*, and ``bot`` and ``trigger`` from version
*4.0* onward. For the purposes of this guide, the *4.0* names will be used.

Your modules
------------

A UnivBot module contains one or more ``callable``\s. It may optionally contain
...
done with decorators, imported from :py:mod:`UnivBot.module`. It may also be
...
.. py:method:: callable(bot, trigger)
    ...
    This is the general function format, called by UnivBot when a command is
    ...
    is that it be callable with a ``UnivBot`` object and a ``Trigger`` object,
    as noted above. Usually, methods of the UnivBot object will be used in reply
    ...
    :py:mod:`UnivBot.module`, except in version *3.2* in which they are defined
    as attributes of the ``UnivBot`` class.
    ...
    .. py:attribute:: commands
        ...
        *See also:* :py:func:`UnivBot.module.commands`

        A list of commands which will trigger the function. If the UnivBot instance
    ...
    .. py:attribute:: rule
        ...
        *See also:* :py:func:`UnivBot.module.rule`

        A regular expression which will trigger the function. If the UnivBot
    ...
    .. py:attribute:: rate
        ...
        *See also:* :py:func:`UnivBot.module.rate`
...
.. py:method:: setup(UnivBot)
    ...
    the UnivBot object's messaging functions is undefined.

    Throwing an exception from this function (such as a `ConfigurationError
    <#UnivBot.config.ConfigurationError>`_) will prevent any callables in the
...
.. py:method:: shutdown(UnivBot)

    *Availability: 4.1+*

    This is an optional function of a module, which will be called while the
    UnivBot is quitting. Note that this normally occurs after closing connection
    to the server, so the behavior of the UnivBot object's messaging functions
...
.. py:function:: quit(message)
    ...
    UnivBot will notify modules that it is quitting should the modules have
    a ``shutdown`` method.
...
.. py:attribute:: nick
    ...
    UnivBot's current nick. Changing this while UnivBot is running is unsupported.

.. py:attribute:: name

    UnivBot's "real name", as used for whois.

.. py:attribute:: password

    UnivBot's NickServ password

.. py:attribute:: channels

    A list of UnivBot's initial channels. This list will initially be the same
...
.. py:function:: write(args, text=None)
    ...
    In other words, both ``UnivBot.write(('PRIVMSG',), 'Hello, world!')``
    and ``UnivBot.write(('PRIVMSG', ':Hello, world!'))`` will send
   ...
   .. py:function:: msg(recipient, text, max_messages=1)
    ...
    UnivBot from being caught in an infinite loop with another bot, or being
    used to spam.
    ...
    determinable by UnivBot (though you can generally rely on 400 bytes
    making it through).
...
.. py:function:: debug(tag, text, level)
    ...
    *Availability: 3+*
    ...
    Send ``text`` to UnivBot's configured ``debug_target``. This can be either
    an IRC channel (starting with ``#``) or ``stdio``. Suppress the message
    if the given ``level`` is lower than UnivBot's configured ``verbose``
    setting. Acceptable values for ``level`` are ``'verbose'`` (only send if
    UnivBot is in verbose mode), ``'warning'`` (send if UnivBot is in verbose
...
.. py:class:: Trigger
    ...
    .. py:attribute:: admin
        ...
        True if the nick which triggered the command is in UnivBot's admin list as
...
The ``UnivBot`` object has, among others, the attributes ``db`` and
``config``. These can be used for a number of functions and features.

The ``UnivBotDB`` class
----------------------

.. automodule:: UnivBot.db
   :members:

The ``Config`` class
--------------------

.. automodule:: UnivBot.config
   :members:
   :undoc-members:
...
.. automodule:: UnivBot.web
   :members:

UnivBot.tools
------------

.. automodule:: UnivBot.tools
   :members:

UnivBot.module
-------------
.. automodule:: UnivBot.module
   :members:
